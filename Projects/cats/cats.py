...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    suitable = [p for p in paragraphs if select(p)]
    if k < len(suitable):
        return suitable[k]
    else:
        return ""
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def check_key_word(p):
        check = False
        word_list = [lower(word) for word in split(remove_punctuation(p))]
        for key in subject:
            if key in word_list:
                check = True
        return check

    return check_key_word
    # END PROBLEM 2
        ...
        if not len(typed_words):
            ...
            for index in range(len(typed_words)):
                if (
                    ...
                    and typed_words[index] == source_words[index]
                ):
                    correct_count += 1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_dic = {
            i: diff_function(typed_word, word_list[i], limit)
            for i in range(len(word_list))
        }
        sub_word_index = min(diff_dic, key=lambda i: diff_dic[i])
        if diff_dic[sub_word_index] > limit:
            return typed_word
        else:
            return word_list[sub_word_index]
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit < 0:
        return limit + 1
    elif len(source) * len(typed) == 0:  # Fill in the condition
        # BEGIN
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            break
    dict_progress = {"id": user_id, "progress": count / len(prompt)}
    upload(dict_progress)
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for player_list in times_per_player:
        diff_list = []
        for time_index in range(len(player_list) - 1):
            diff_list.append(player_list[time_index + 1] - player_list[time_index])
        times.append(diff_list)
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fast_list = []
    for player_ini in player_indices:
        fast_list.append([])

    for word_index in word_indices:
        temp_word_dict = {i: time(match, i, word_index) for i in player_indices}
        fast_list[min(temp_word_dict, key=lambda i: temp_word_dict[i])].append(
