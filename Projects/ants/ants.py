...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        position = 0
        while not place.is_hive and not bee:
            if self.lower_bound <= position <= self.upper_bound:
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def difficult_action(gamestate):
            if target.slowed_turns <= 0 or gamestate.time % 2 == 0:
                Bee.action(target, gamestate)
                target.slowed_turns -= 1
            else:
                target.slowed_turns -= 1

        target.action = difficult_action
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not curr_place.is_hive:
            ...
            if curr_ant and curr_ant is not self:
                insects[curr_ant] = distance
                if (
                    curr_ant.is_container
                    and curr_ant.ant_contained
                    and curr_ant.ant_contained is not self
                ):
                    insects[curr_ant.ant_contained] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        actual_damage = self.damage - 0.0625 * self.insects_shot - 0.25 * distance
        return actual_damage if actual_damage > 0 else 0
        # END Problem Optional 2
