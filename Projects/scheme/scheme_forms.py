...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        func_expressions = Pair(signature.rest, expressions.rest)
        procedure = do_lambda_form(func_expressions, env)
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        val = scheme_eval(expressions.first, env, expressions.rest == nil)
...
def do_define_macro(expressions, env):
    ...
    if not scheme_listp(signature):
        raise SchemeError("it is not Scheme list")
    name = signature.first
    if scheme_numberp(name):
        raise SchemeError("we couldn't pass number")
