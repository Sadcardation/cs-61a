...
def balanced(m):
    ...
    True
    """
    # Set balanced of planet is True, `balanced(l) and balanced(r)` for passing to next layer.
    # if is_planet(m):
    #     return True
    ...
    # Considering the "most" base case, the base case in code above is a planet, the base case in code below is
    # a mobile with two planets at both arms which is not the "most" base case.
...
def sub_interval(x, y):
    ...
    and any value in y."""
    temp = interval(-upper_bound(y), -lower_bound(y))
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert upper_bound(y) * lower_bound(y) > 0, "it is not clear what it means to divide by an interval that spans zero"
