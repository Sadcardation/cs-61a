...
def subseqs(s):
    ...
    if not s:
        ...
        item = s[0]
        return subseqs(s[1:]) + insert_into_all(item, subseqs(s[1:]))
...
def non_decrease_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = s[0]
            b = prev
            return insert_into_all(a, subseq_helper(s[1:], a)) + subseq_helper(s[1:], b)
...
def long_paths(t, n):
    ...
    """
    result = []
    min_len = lambda x: len(x) - 1 >= n

    def all_paths(t, curr_path):
        curr_path.append(t.label)
        if t.is_leaf():
            result.append(curr_path)
        else:
            for branch in t.branches:
                all_paths(branch, list(curr_path))

    all_paths(t, [])
    return list(filter(min_len, result))
    ...
    def reverse(t, depth):
        if not t.is_leaf():
            if depth % 2:
                ...
                ele = [b.label for b in t.branches]
                for branch in t.branches:
                    branch.label = ele.pop()
                    reverse(branch, depth + 1)
