...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        True
        """
        player.hand.extend([player.deck.draw() for _ in range(2)])
        implemented = True
...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        added = False
        if player.hand:
            cloned_card = player.hand[0].copy()
            player.hand.append(cloned_card)
            added = True
        # You should add your implementation above this.
        if added:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if player.hand:
            power_list = [card.power(opponent_card) for card in player.hand]
            index = power_list.index(max(power_list))
